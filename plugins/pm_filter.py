...
async def msg_handler(c, m):
    ...
    if user_input:
        requested_name = user_input.replace(' ', '+')
        url = f"https://letsdash.online/search?search={requested_name}"
        button_url = InlineKeyboardButton(text="Visit Let's Dash!", url=url)
        inline_keyboard = InlineKeyboardMarkup([[button_url]])
        await m.reply_text(
            f"Here is the Let's Dash URL for '{user_input}': {url}", 
            reply_markup=inline_keyboard
        )
    else:
        await m.reply_text("Please enter a movie name.")
